document.addEventListener("DOMContentLoaded", function () {
  ...
  window.borrarCookie = function () {
    const clave = document.getElementById("cookiesSelclave").value; //importante .value
  ...
  function actualizarClaves(almacenamiento) {
    ...
    const selectElement = document.getElementById(`${almacenamiento}Selclave`); //en html los id se llaman localSelclave por ejemplo
    selectElement.innerHTML = "";

    const resultListElement = document.getElementById(
      `resultList${
        almacenamiento.charAt(0).toUpperCase() + almacenamiento.slice(1)
      }`
    );
  ...
  actualizarClaves("local"); //Se llamara a actualizarClaves(localSelclave)
  actualizarClaves("session"); //''
  actualizarClaves("cookies"); //''
});
